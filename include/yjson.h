...
class YJson final
{
  ...
  private:
    ...
    template <typename StrIterator> static inline StrIterator StrSkip(StrIterator first, StrIterator last)
    {
  ...
  public:
    ...
    template <typename _Iterator> inline YJson(_Iterator first, _Iterator last)
    {
    ...
    inline YJson::Type getType() const
    {
        return _type;
    }

    inline std::u8string &getValueString()
    {
        return *_value.String;
    }
    inline const std::u8string &getValueString() const
    {
        return *_value.String;
    }
    inline int32_t getValueInt()
    {
    ...
    template <typename _Iterator> void assignA(_Iterator first, _Iterator last)
    {
    ...
    template <typename _CharT> static std::basic_string<_CharT> pureUrlEncode(const std::basic_string_view<_CharT> str)
    {
    ...
    template <typename _Ty = std::u8string> static std::u8string pureUrlDecode(const std::u8string &str)
    {
    ...
    template <typename _Iterator> inline void setText(_Iterator first, _Iterator last)
    {
        if (_type != YJson::String)
        {
            clearData();
    ...
    template <typename _Ty> inline void setText(const _Ty &utf8Array)
    {
        if (_type != YJson::String)
        {
            clearData();
    ...
    ArrayIterator findByValA(double value)
    {
        return std::find_if(_value.Array->begin(), _value.Array->end(), [value](const YJson &item) {
            return item._type == YJson::Number &&
                   fabs(value - *item._value.Double) <= std::numeric_limits<double>::epsilon();
        });
    }
    ...
    template <typename _Ty = const std::u8string_view> inline ArrayIterator append(_Ty value)
    {
    ...
    template <typename _Ty> inline ArrayIterator removeByValA(_Ty str)
    {
    ...
    inline ObjectIterator find(const std::u8string_view key)
    {
        return std::find_if(_value.Object->begin(), _value.Object->end(),
                            [&key](const YJson::ObjectItemType &item) { return item.first == key; });
    }
    ...
    inline ObjectIterator findByValO(double value)
    {
        return std::find_if(_value.Object->begin(), _value.Object->end(), [&value](const YJson::ObjectItemType &item) {
            return item.second._type == YJson::Number &&
                   fabs(value - *item.second._value.Double) <= std::numeric_limits<double>::epsilon();
        });
    }
    inline ObjectIterator findByValO(const std::u8string_view str)
    {
        return std::find_if(_value.Object->begin(), _value.Object->end(), [&str](const YJson::ObjectItemType &item) {
            return item.second._type == YJson::String && *item.second._value.String == str;
        });
    }
    ...
    template <typename _Ty> inline ObjectIterator removeByValO(_Ty str)
    {
    ...
    inline ArrayIterator beginA()
    {
        return _value.Array->begin();
    }
    inline ObjectIterator beginO()
    {
        return _value.Object->begin();
    }
    inline ArrayIterator endA()
    {
        return _value.Array->end();
    }
    inline ObjectIterator endO()
    {
        return _value.Object->end();
    }
    inline ObjectItemType frontO()
    {
        return _value.Object->front();
    }
    inline ObjectItemType backO()
    {
        return _value.Object->back();
    }
    inline ArrayItemType frontA()
    {
        return _value.Array->front();
    }
    inline ArrayItemType backA()
    {
  ...
  private:
    ...
    template <typename StrIterator> StrIterator parseValue(StrIterator first, StrIterator last)
    {
        if (last <= first)
            return last;
        if (*first == '\"')
    ...
    template <typename StrIterator> StrIterator parseNumber(StrIterator first, StrIterator)
    {
        _type = YJson::Number;
        _value.Double = new double(0);
        short sign = 1;
    ...
    template <typename T> inline static bool _parse_hex4(T str, uint16_t &h)
    {
        if (*str >= '0' && *str <= '9')
            h += (*str) - '0';
        else if (*str >= 'A' && *str <= 'F')
    ...
    template <typename T> inline static uint16_t parse_hex4(T str)
    {
        uint16_t h = 0;
        if (_parse_hex4(str, h) || _parse_hex4(++str, h = h << 4) || _parse_hex4(++str, h = h << 4) ||
            _parse_hex4(++str, h = h << 4))
            return 0;
        return h;
    }
        ...
        while (ptr != last && *ptr != '\"')
        {
            ...
            else
            {
                ...
                case 'u':
                    ...
                    if ((uc >= utf16FirstWcharMark[1] && uc < utf16FirstWcharMark[2]) || uc == 0)
    ...
    template <typename StrIterator> StrIterator parseArray(StrIterator first, StrIterator last)
    {
        _type = YJson::Array;
