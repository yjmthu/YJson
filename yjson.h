...
public:
    ...
    inline YJson& operator[](int i) const { auto j = find(i); if (!j) throw std::errc::invalid_seek; else return *j; }
    inline YJson& operator[](const char* key) const { auto j = find(key); if (!j) throw std::errc::invalid_seek; else  return *j; }
    inline YJson& operator[](const std::string& key) const { auto j = find(key); if (!j) throw std::errc::invalid_seek; else  return *j; }
    ...
    inline void setValue(bool val) {delete _child; _child = nullptr; delete [] _value; _value = nullptr; _type = val? True: False;}
